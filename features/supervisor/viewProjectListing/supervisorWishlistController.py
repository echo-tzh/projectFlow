...
supervisor_wishlist_bp = Blueprint(
    "supervisor_wishlist",
    __name__,
...
@supervisor_wishlist_bp.route("/wishlist/add", methods=["POST"])
@login_required
def add_to_supervisor_wishlist():
    """Add a project to the supervisor's wishlist."""
    try:
...
@supervisor_wishlist_bp.route("/wishlist/remove", methods=["POST"])
@login_required
def remove_from_supervisor_wishlist():
    """Remove a project from the supervisor's wishlist."""
    try:
        data = request.get_json()
        project_id = data.get('project_id')
...
@supervisor_wishlist_bp.route("/wishlist", methods=["GET"])
@login_required
def get_supervisor_wishlist():
    """Get all projects in the supervisor's wishlist."""
    try:
        user = User.query.get(session["user_id"])
        if not user:
            return jsonify({'success': False, 'message': 'User not found'}), 404
